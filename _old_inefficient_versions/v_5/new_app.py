...
def process_file():
    ...
    WHEN = dropdown_var.get()
    log_text.insert(tk.END, f"Selected option: {WHEN}\n")
    ...
    if not WHEN:
        log_text.insert(tk.END, "Please select an option first.\n")
        return
    ...
    if WHEN =='SCHOOL_YEAR':
        template_path = dictionary['template_path_SCHOOL_YEAR']
    elif WHEN == 'SUMMER':
        template_path = dictionary['template_path_SUMMER']
    ...
    create_schedule(cleaning_path,create_appointments_path,missing_students,WHEN)
...
if __name__ == "__main__":
    ...
    # Create a Combobox (drop-down menu) for additional options
    options = ["SCHOOL_YEAR", "SUMMER"]
    dropdown_var = tk.StringVar()
    dropdown_menu = ttk.Combobox(root, textvariable=dropdown_var, values=options)
    dropdown_menu.set("Select an option")
    dropdown_menu.pack()
