...
def create_list(selected_file_path):
    ''' Creates a list of two lists. One list is for the online students the other
...
def get_cells_ol(lst,day_num,WHEN):
    ...
    if WHEN == 'SUMMER':
        if  (weekday == 5) or (weekday == 4):
            ...
            initial_time = datetime.time(hour = 15, minute = 00)
        initial_column = 'C'
...
def get_cell(lst,day_num,WHEN):
    ...
    num_columns = 100  # Specify the number of columns you need
    ...
    if WHEN == 'SCHOOL_YEAR':
        ...
        a = datetime.time(hour = 15)
        b = datetime.time(hour = 15, minute = 5)
        c = datetime.time(hour = 15, minute = 10)
        d = datetime.time(hour = 15, minute = 15)
        e = datetime.time(hour = 15, minute = 30)
...
def create_schedule(selected_file_path,file_path,missing_students,WHEN):
