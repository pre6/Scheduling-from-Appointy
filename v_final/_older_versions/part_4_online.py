...
with pd.ExcelWriter("test.xlsx", engine='openpyxl') as writer:
    for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']:
        ...
        schedule['Note 3'] = [''] * len(time_slots)
        schedule['Note 4'] = [''] * len(time_slots)
        schedule['Note 5'] = [''] * len(time_slots)
            ...
            if not placed:
                ...
                for i in range(1, num_slots):
                    if start_idx + i < len(schedule):
                        schedule.at[start_idx + i, col] = 'BLOCKED'
                group_id = (len(online_col) - 1) % color_groups
                bookings.append((col, start_idx, num_slots, group_id))
                
        
                
        styled_schedule = schedule.style.map(color_red)      
        table_columns = []        
        ...
        reordered_cols = ['Note 1', 'Note 2', 'Time', 'Note 3', 'Note 4', 'Note 5']+online_col
...
for sheet_name in wb.sheetnames:
    ws = wb[sheet_name]
    schedule_df = pd.read_excel("test.xlsx", sheet_name=sheet_name)
